...
public class AbstractHashedMap extends AbstractMap implements IterableMap {
	 ...
	 */
	protected void removeMapping(HashEntry entry, int hashIndex,
			HashEntry previous) {
		modCount++;
	...
	protected static class HashEntry implements Map.Entry, KeyValue {
		...
		protected HashEntry(HashEntry next, int hashCode, Object key,
				Object value) {
			super();
			this.next = next;
		...
		public String toString() {
			return new StringBuffer().append(getKey()).append('=').append(
					getValue()).toString();
		}
	}
	...
	protected static abstract class HashIterator implements Iterator {
		...
		public String toString() {
			if (last != null) {
				return "Iterator[" + last.getKey() + "=" + last.getValue() //$NON-NLS-1$ //$NON-NLS-2$
						+ "]"; //$NON-NLS-1$
			} else {
	...
	public String toString() {
		...
		while (hasNext) {
			Object key = it.next();
			Object value = it.getValue();
			buf.append(key == this ? "(this Map)" : key).append('=').append( //$NON-NLS-1$
					value == this ? "(this Map)" : value); //$NON-NLS-1$
