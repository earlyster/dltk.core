...
public class BuildParticipantManager extends NatureExtensionManager {
	...
	private static final String EXT_POINT = "org.eclipse.dltk.validators.core.buildParticipant"; //$NON-NLS-1$
	...
	protected void registerConfigurationElements() {
		super.registerConfigurationElements();
		// compatibility: load entries from previous extension point
		registerConfigurationElements(
				Platform.getExtensionRegistry().getConfigurationElementsFor(
						ValidatorManager.LANGUAGE_EXTPOINT), "nature"); //$NON-NLS-1$
	}
	...
	public static IBuildParticipant[] getBuildParticipants(
		...
		for (int i = 0; i < factories.length; ++i) {
			try {
				...
				ValidatorsCore.warn(NLS.bind(tpl, factories[i].getName()), e);
			}
		}
